...
const BookingPage = () => {
  ...
  const isHealthFormComplete = useMemo(() => {
    const radioQuestions = [
      'everDonated', 'hasCurrentIllness', 'had12mDiseases', 'had12mTransfusion', 'had12mRabiesVax'
    ];
            ...
            <CardContent className="space-y-1">
                ...
                {healthFormData.isFemale && (
                  <div className="pl-6 space-y-1">
                    <QuestionRow question="6. Hiện có thai, hoặc nuôi con dưới 12 tháng tuổi?"><RadioGroup onValueChange={(v) => handleFormChange('isPregnant', v)} value={healthFormData.isPregnant} className="flex space-x-6"><div className="flex items-center space-x-2"><RadioGroupItem value="yes" id="qf1-yes" /><Label htmlFor="qf1-yes">Có</Label></div><div className="flex items-center space-x-2"><RadioGroupItem value="no" id="qf1-no" /><Label htmlFor="qf1-no">Không</Label></div></RadioGroup></QuestionRow>
                    <QuestionRow question="7. Có kinh nguyệt trong vòng một tuần hay không?"><RadioGroup onValueChange={(v) => handleFormChange('hadPeriod', v)} value={healthFormData.hadPeriod} className="flex space-x-6"><div className="flex items-center space-x-2"><RadioGroupItem value="yes" id="qf2-yes" /><Label htmlFor="qf2-yes">Có</Label></div><div className="flex items-center space-x-2"><RadioGroupItem value="no" id="qf2-no" /><Label htmlFor="qf2-no">Không</Label></div></RadioGroup></QuestionRow>
                  </div>
                )}
                ...
                <div className="flex items-start space-x-3 pt-6 border-t mt-4">
                    ...
                    8. Anh/chị có đồng ý xét nghiệm HIV, nhận thông báo và được tư vấn khi kết quả xét nghiệm HIV nghi ngờ dương tính?
                    </Label>
                </div>
            </CardContent>
          </Card>
        );
      case 2: // This is now the "Review & Confirm" step
        return (
          <Card className="bg-white shadow-md-custom rounded-md-custom">
      ...
      <div className="max-w-7xl mx-auto px-3 md:px-5 lg:px-10 py-xl">
        ...
        <div className="grid lg:grid-cols-3 gap-xl">
          <div className="lg:col-span-1 space-y-l">
            ...
            {/* Time Slot moved to the left column */}
            {currentStep === 1 && (
              <Card className="bg-white shadow-md-custom rounded-md-custom">
            ...
            <div className="flex gap-4">
                {currentStep < 2 ? (
                    <Button size="lg" onClick={handleNext} className="bg-compassion-red hover:bg-compassion-red/90 text-white rounded-md-custom transition-all duration-300 hover:scale-105"
                        disabled={!isHealthFormComplete || !selectedTimeSlot}>
                        Bước tiếp theo →
                    </Button>
                ) : (
